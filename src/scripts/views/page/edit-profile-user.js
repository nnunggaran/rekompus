const EditProfileUser = {
  async render() {
    return `
    <div class="hero-text container-fluid">
      <h1 class="text-center text-white">Atur data dirimu!</h1>
    </div>
    <div class="bg-info p-3">
      <div class="head-profile container-fluid mt-2">
        <div class="card border-orange bg-light">
          <div class="row alert-info m-3">
            <div class="col-md-2 p-3">
              <img src="./favicon.png" alt="" class="thumb-img-detail">
            </div>
            <div class="col-md-5 p-3 my-auto ml-auto">
              <h4 class="fw-bold text-muted">Nama User</h4>
              <p class="fw-bold text-muted">Siswa di SMAN 1 Aceh</p>
            </div>
            <div class="col-md-5 p-3 my-auto">
              <a href="/#/edit-profile/:id" class="link text-dark text-decoration-none border-bottom border-info border-2 fw-bold fs-5 py-2">
                <i class="fa fa-edit fa-2x text-muted" aria-hidden="true"></i> <span class="text-muted ">Atur data
                  dirimu!</span>
              </a>
            </div>
          </div>
        </div>

      </div>
      <form id="formUser">
      <div class="container-fluid mt-3">
        <input type="hidden" id="idUser" value="id">
        <div class="card">
          <div class="card-header alert-info">
            <h4 class="text-heading text-muted fw-bolder">Akun</h4>
          </div>
          <div class="card-body">
            <div class="card border-orange mb-3 alert-info">
              <div class="d-flex p-3 justify-content-between align-items-center">
                <label for="email"><h5 class="fw-bold text-muted">Email</h5></label>
                <input type="email" class="form-control border-orange w-75 ms-4" id="email" placeholder="Email user">
              </div>
            </div>
            <div class="card border-orange mb-3 alert-info">
              <div class="d-flex p-3 justify-content-between align-items-center">
                <label for="password"><h5 class="fw-bold text-muted">Password</h5></label>
                <input type="password" class="form-control border-orange w-75 ms-4" id="password" placeholder="Password user">
              </div>
            </div>        
          </div>
        </div>
      </div>
      <div class="container-fluid mt-3">
        <div class="card">
          <div class="card-header alert-info">
            <h4 class="text-heading text-muted fw-bolder">Biodata</h4>
          </div>
          <div class="card-body">
            <div class="card border-orange mb-3 alert-info">
              <div class="d-flex p-3 justify-content-between align-items-center">
                <label for="namaLengkap"><h5 class="fw-bold text-muted">Nama lengkap</h5></label>
                <input type="text" class="form-control border-orange w-75 ms-4" id="namaLengkap" placeholder="Nama lengkap user">
              </div>
            </div>
            <div class="card border-orange mb-3 alert-info">
              <div class="d-flex p-3 justify-content-between align-items-center">
                <label for="asalSekolah"><h5 class="fw-bold text-muted">Asal sekolah</h5></label>
                <input type="text" class="form-control border-orange w-75 ms-4" id="asalSekolah" placeholder="Asal sekolah">
              </div>
            </div>
            <div class="card border-orange mb-3 alert-info">
              <div class="d-flex p-3 justify-content-between align-items-center">
                <label for="fotoProfil"><h5 class="fw-bold text-muted">Foto Profil</h5></label>
                <input type="file" name="fotoProfil" class="form-control border-orange w-75 ms-4" id="fotoProfil">
              </div>
            </div>
            <div class="text-end">
              <button type="submit" class="btn btn-dblue fs-4"><i class="fa fa-save fa-lg" aria-hidden="true"></i> Simpan</button>  
            </div>
          </div>
        </div>
      </div>
      </form>
    </div>
    `;
  },

  async afterRender() {
    const formUser = document.querySelector('#formUser');
    // const txtId = document.querySelector('#idUser');
    // const txtEmail = document.querySelector('#email');
    // const txtPassword = document.querySelector('#password');
    // const txtNamaLengkap = document.querySelector('#namaLengkap');
    // const txtAsalSekolah = document.querySelector('#asalSekolah');
    // const fileFotoProfil = document.querySelector('#fotoProfil');

    formUser.addEventListener('submit', async (e) => {
      e.preventDefault();
    });
  },
};

export default EditProfileUser;
